...
exports.main = async (event: S3Event) => {
    ...
    const res = await addToFirestore(config.firestore, visionResult, keyword, publicImageUrl);
...
async function addToFirestore(firestore: Firestore, visionResult: VisionResult, keyword: string, imageUrl: string): Promise<FirebaseFirestore.DocumentReference<FirebaseFirestore.DocumentData>> {
